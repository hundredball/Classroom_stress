...
class StressData:
    ...
    def __init__(self, EEG_list, labels, df_all):
        ...
        self.labels = labels
        self.df_all = df_all
        self.sample_indices = list(range(len(EEG_list)))  # Used for referencing data in sliding window
    ...
    def remove_trials(self, remove_indices=None):
        ...
        self.sample_indices = list(range(len(self.EEG_list)))
    ...
    def sliding_window_augmentation(self, window_size=10 * cfg.fs):
        ...
        new_EEG_list = []
        new_label_list = []
        num_window = min([data.shape[1] for data in
                          self.EEG_list]) // window_size  # Get number of window based on data with shortest length

        for i_data, EEG in enumerate(self.EEG_list):
            for i in range(num_window):
                new_EEG_list.append(EEG[:, i * window_size:(i + 1) * window_size])
            new_label_list.extend([self.labels[i_data]]*num_window)

        print('Sliding window augments number of data from {old_num} to {new_num} ({multiplier}x)'.format(
            old_num=len(self.EEG_list), new_num=len(new_EEG_list), multiplier=num_window
        ))
        self.EEG_list = new_EEG_list
        self.labels = np.array(new_label_list)
        return num_window
...
if __name__ == '__main__':
    X, Y, df_all = dataloader.read_data(label_format=2, data_folder='resting_ASR_ref')
    data = StressData(X, Y, df_all)
    num_window = data.sliding_window_augmentation()
    print('len(data.EEG_list): ', len(data.EEG_list))
